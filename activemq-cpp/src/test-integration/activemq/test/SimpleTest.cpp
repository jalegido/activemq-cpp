...
void SimpleTest::testProducerSendToNonDefaultDestination() {
    ...
    unique_ptr<cms::TextMessage> txtMessage(session->createTextMessage("TEST MESSAGE"));
    unique_ptr<cms::Destination> destination(session->createTemporaryTopic());
...
void SimpleTest::testMultipleConnections() {
    ...
    // Create CMS Object for Comms
    unique_ptr<ActiveMQConnectionFactory> factory(new ActiveMQConnectionFactory(cmsProvider->getBrokerURL()));
    unique_ptr<cms::Connection> connection1(factory->createConnection());
    connection1->start();

    unique_ptr<cms::Connection> connection2(factory->createConnection());
    connection2->start();

    CPPUNIT_ASSERT(connection1->getClientID() != connection2->getClientID());

    unique_ptr<cms::Session> session1(connection1->createSession());
    unique_ptr<cms::Session> session2(connection1->createSession());

    unique_ptr<cms::Topic> topic(session1->createTopic(UUID::randomUUID().toString()));

    unique_ptr<cms::MessageConsumer> consumer1(session1->createConsumer(topic.get()));
    unique_ptr<cms::MessageConsumer> consumer2(session2->createConsumer(topic.get()));

    unique_ptr<cms::MessageProducer> producer(session2->createProducer(topic.get()));
    producer->setDeliveryMode(DeliveryMode::NON_PERSISTENT);

    unique_ptr<cms::TextMessage> textMessage(session2->createTextMessage());

    // Send some text messages
    producer->send(textMessage.get());

    unique_ptr<cms::Message> message(consumer1->receive(2000));
    CPPUNIT_ASSERT(message.get() != NULL);
...
void SimpleTest::testMultipleSessions() {
    ...
    // Create CMS Object for Comms
    unique_ptr<cms::Session> session1(cmsProvider->getConnection()->createSession());
    unique_ptr<cms::Session> session2(cmsProvider->getConnection()->createSession());
...
void SimpleTest::testReceiveAlreadyInQueue() {
    ...
    unique_ptr<cms::Session> session(connection->createSession());
    unique_ptr<cms::Topic> topic(session->createTopic(UUID::randomUUID().toString()));
    unique_ptr<cms::MessageConsumer> consumer(session->createConsumer(topic.get()));
    unique_ptr<cms::MessageProducer> producer(session->createProducer(topic.get()));
    producer->setDeliveryMode(DeliveryMode::NON_PERSISTENT);
    unique_ptr<cms::TextMessage> textMessage(session->createTextMessage());
